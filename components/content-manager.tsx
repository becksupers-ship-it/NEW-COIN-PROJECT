...
export function ContentManager({ mode }: { mode: Mode }) {
  ...
  return <form onSubmit={submit} className="mt-8 grid gap-4 rounded-[2rem] border border-[#d9e1d8] bg-white p-6 shadow-sm">
    ...
    {mode === 'news' && <><label className="grid gap-2 text-sm font-bold">Featured image<input ref={fileRef} type="file" accept="image/*,.jpg,.jpeg,.heic,.heif,.avif,.dng,.png,.webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadImage(file) }} className="rounded-xl border border-[#cbd8cc] p-3 font-normal" /></label>{imageUrl && <img src={imageUrl} alt="Selected post preview" className="max-h-56 w-full rounded-2xl object-cover" />}</>}
