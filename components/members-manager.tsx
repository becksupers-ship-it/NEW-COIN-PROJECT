...
export function MembersManager({ initialMembers }: { initialMembers: Member[] }) {
  ...
  return <div className="grid gap-8 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
    <form onSubmit={save} className="rounded-[2rem] border border-[#d9e1d8] bg-[#fffdf8] p-6 shadow-sm lg:p-8">
      <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#c56b4b]">Member profile</p><h2 className="mt-2 text-3xl font-semibold">{editingId ? 'Edit member' : 'Add verified member'}</h2></div>{editingId && <button type="button" onClick={() => { setEditingId(null); setForm(emptyForm) }} className="text-sm font-bold text-[#c56b4b]">Cancel</button>}</div>
      ...
      <button disabled={busy || uploading} className="mt-7 w-full rounded-full bg-[#c56b4b] px-5 py-4 font-bold text-white disabled:opacity-60">{busy ? 'Saving…' : editingId ? 'Save member changes' : 'Add verified member'}</button>
      {message && <p className="mt-4 rounded-xl bg-[#edf4ed] p-3 text-sm font-semibold">{message}</p>}
    </form>
    <section><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#c56b4b]">Directory</p><h2 className="mt-2 text-3xl font-semibold">Verified members</h2></div><span className="text-sm text-[#64776e]">{members.length} total</span></div><div className="mt-5 grid gap-4 sm:grid-cols-2">{members.length ? members.map((member) => <article key={member.id} className="overflow-hidden rounded-[1.5rem] border border-[#d9e1d8] bg-white"><div className="flex gap-4 p-5">{member.imageUrl ? <img src={member.imageUrl} alt={member.name} className="h-20 w-20 rounded-2xl object-cover" /> : <div className="grid h-20 w-20 shrink-0 place-items-center rounded-2xl bg-[#e5b65c] text-2xl font-bold text-[#17332d]">{member.name.slice(0, 1)}</div>}<div className="min-w-0"><h3 className="font-bold">{member.name}</h3><p className="mt-1 text-sm text-[#c56b4b]">{member.office}</p><p className="mt-1 text-xs text-[#64776e]">{member.published ? 'Published' : 'Hidden'}</p></div></div><div className="border-t border-[#edf1eb] px-5 py-3 text-sm text-[#64776e]">{member.occupation} · {member.currentState}</div><div className="flex gap-2 px-5 pb-5"><button type="button" onClick={() => edit(member)} className="rounded-full border border-[#cbd8cc] px-4 py-2 text-sm font-bold">Edit</button><button type="button" onClick={() => void remove(member.id)} className="rounded-full border border-[#e3b7a9] px-4 py-2 text-sm font-bold text-[#a94f37]">Remove</button></div></article>) : <p className="rounded-3xl border border-dashed border-[#cbd8cc] p-8 text-[#64776e]">No verified members have been added yet.</p>}</div></section>
  </div>
}
